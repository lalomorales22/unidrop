...
UniDrop v0.1 is designed for direct file sharing between computers on the same trusted or semi-trusted local network. It encrypts transfers and requires an explicit first pairing. It is not yet an audited replacement for AirDrop in a hostile enterprise network.
...
- The browser panel has no native tray/menu-bar receiver-approval prompt yet.
